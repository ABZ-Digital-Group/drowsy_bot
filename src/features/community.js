...
function createCommunityFeature({ client, config, state, helpers, stageFeature }) {
    ...
    function buildSparkline(values) {
        ...
        return values.map(value => {
            const index = Math.min(bars.length - 1, Math.round((value / max) * (bars.length - 1)));
            return bars[index];
        }).join('');
    }
    ...
    async function refreshConfiguredStatsForGuild(guild, options = {}) {
        ...
        for (const [metric, channelId] of bindings) {
            ...
            await channel.setName(nextName, 'Refreshing server stats.').catch(() => {
                missingPermissions += 1;
            });
    ...
    async function handleVoiceStateUpdate(oldState, newState) {
        ...
        if (!wasBot && oldState.channelId !== newState.channelId) {
            if (oldState.channelId) {
                finalizeVoiceSession(guild.id, memberId);
            }

            if (newState.channelId) {
                beginVoiceSession(guild.id, memberId, newState.channelId);
            }
        }
